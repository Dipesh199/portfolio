    ...
    >
      ...
      <div className="p-6 pt-12 relative">
        ...
        <motion.div
          ...
          className={`absolute -top-10 left-6 w-20 h-20 rounded-2xl bg-gradient-to-br ${game.theme.primary} p-1 shadow-2xl ${game.theme.glow}`}
        >
          <img src={game.icon} alt={game.title} className="w-full h-full rounded-[14px] object-cover" />
        </motion.div>
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm text-gray-400 font-medium">{game.category}</p>
        </div>

        {/* Hover Glow Effect */}
        <div className={`absolute inset-0 opacity-0 group-hover:opacity-10 rounded-3xl transition-opacity duration-500 bg-gradient-to-br ${game.theme.primary}`} />
      </div>
